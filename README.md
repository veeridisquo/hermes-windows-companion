...
Each profile has a submenu: open a terminal running `hermes -p <name> chat`, open a terminal ready for that profile's commands, view its details, open its folder, and start or stop its gateway. Terminals open in Windows Terminal when it is installed, and in a console window otherwise.
